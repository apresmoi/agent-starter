...
(async () => {
  ...
  async function checkForMention(state: typeof StateAnnotation.State) {
    const { content, authorId } = state.message;
    const knownSenders = state.knownSenders;

    // 1️⃣ Is it a mention?
    const mentioned = content.includes(`${MY_ID}`) || content.includes(`${MY_NAME}`);

    // 2️⃣ Have we answered this author before?
    const firstTime = !knownSenders.has(authorId);

    return mentioned && firstTime ? 'reply' : 'finish';
  }

  async function nodeReplyToMention(state: typeof StateAnnotation.State) {
    const { authorId } = state.message;
    const knownSenders = state.knownSenders;
    // Quote + reply
    const replyBody = `👋 Hey ${authorId}, nice to meet you!`;
    ...
    console.log(`💬 Replied to first mention from ${authorId}`);
  ...
  const graph = new StateGraph(StateAnnotation)
    .addNode('replyToMention', nodeReplyToMention)
    .addConditionalEdges(START, checkForMention, {
      reply: 'replyToMention',
      finish: END,
    })
    .compile();
