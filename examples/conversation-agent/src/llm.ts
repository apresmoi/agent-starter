...
const messageReviewSchema = z.object({
  ...
  like: z
    .boolean()
    .describe(
      "Whether the bot would 'like' this message based on its persona.",
    ),
  dislike: z
    .boolean()
    .describe(
      "Whether the bot would 'dislike' this message based on its persona.",
    ),
...
Respond using the structured output format.`,
  ...
  try {
    ...
    if (review.like) {
      reaction =
        personality.reactions.like[
        Math.floor(Math.random() * personality.reactions.like.length)
        ];
    ...
    logger.error(
      `[LLM] Error during LLM review for message ID ${message.id}:`,
      error,
    );
    return {
      ...message,
...
${[`- Author: ${msg.authorId}`,
        ...
        msg.like && msg.positiveAlternativeThought ? `- Positive thought you had about this message: ${msg.positiveAlternativeThought}` : "",
        msg.dislike && msg.negativeAlternativeThought ? `- Critical thought you had about this message: ${msg.negativeAlternativeThought}` : "",
        msg.randomThought ? `- Random thought you had while reviewing this message: ${msg.randomThought}` : ""
        ].filter(Boolean).join("\n")}
-----
`;
    })
...
${getReplyGenerationGuidelines(personality)}`,
  ...
  const yourLastNMessages = yourMessages.slice(-personality.memory_length);
  logger.debug(
    `[LLM] Using last ${personality.memory_length} bot messages for context`,
  );

  // If we have previous messages, build the history
  if (yourLastNMessages.length > 0) {
    for (let i = 0; i < yourLastNMessages.length; i++) {
      const messageIndex = yourLastNMessages[i].index;
      const nextMessageIndex =
        yourLastNMessages[i + 1]?.index ?? messages.length;
      ...
      logger.debug(
        `[LLM] Processing message group ${i + 1}/${yourLastNMessages.length}, found ${otherMessages.length} other messages`,
      );
      ...
      if (i < yourLastNMessages.length - 1) {
        historyMessages.push(
          new AIMessage({
            content: yourLastNMessages[i + 1].message.content,
          }),
        );
      }
    }
    ...
    logger.debug(
      "[LLM] No previous bot messages found, treating all messages as initial input",
    );
    // Bot has no messages in yourLastNMessages (e.g., first reply, or history is clear)
  ...
  logger.debug(
    "[LLM] Built conversation history with messages:",
  ...
  try {
    ...
    const result =
      replyContent && replyContent !== "NO_REPLY" ? replyContent : "";
    logger.debug("[LLM] Generated reply:", result);
    return result;
  } catch (error) {
    logger.error(
      `[LLM] Error generating or sending consolidated reply: `,
      error,
    );
    return "";
  }
