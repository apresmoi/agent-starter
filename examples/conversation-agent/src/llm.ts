...
${[
  ...
  msg.like && msg.positiveAlternativeThought
    ? `- Positive thought you had about this message: ${msg.positiveAlternativeThought}`
    : '',
  msg.dislike && msg.negativeAlternativeThought
    ? `- Critical thought you had about this message: ${msg.negativeAlternativeThought}`
    : '',
  msg.randomThought
    ? `- Random thought you had while reviewing this message: ${msg.randomThought}`
    : '',
]
  .filter(Boolean)
  .join('\n')}
...
${getReplyGenerationGuidelines(personality)}`,
  ...
  if (yourLastNMessages.length > 0) {
    for (let i = 0; i < yourLastNMessages.length; i++) {
      ...
      const nextMessageIndex = yourLastNMessages[i + 1]?.index ?? messages.length;
