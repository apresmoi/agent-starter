...
export class Agent {
  ...
  private _resetTimers() {
    ...
    if (RECONNECT_TIMEOUT_MS > 0) {
      logger.debug(
        `[AGENT] Resetting inactivity timer for ${RECONNECT_TIMEOUT_MS}ms`,
      );
      ...
      logger.debug(
        "[AGENT] Inactivity timer is disabled (RECONNECT_TIMEOUT_MS <= 0).",
      );
    }
    ...
    if (SILENCE_TIMEOUT_MS > 0) {
      logger.debug(
        `[AGENT] Resetting silence timer for ${SILENCE_TIMEOUT_MS}ms`,
      );
  ...
  private async _processMessageBatch() {
    ...
    let otherMessages = this.newMessagesBuffer.filter(
      (msg) => msg.authorId !== this.agentId,
    );
    ...
    logger.debug(
      `[AGENT] Processing message batch of size ${this.newMessagesBuffer.length}`,
    );
    ...
    if (reactionMessages.length > 0) {
      ...
      for (const { id, reaction, authorId, like, dislike } of reactionMessages) {

        // Skip based on reaction type and probability
        if(like && personality.speak_prob_on_like <= Math.random()) continue;
        if(dislike && personality.speak_prob_on_dislike <= Math.random()) continue;
    ...
    logger.debug(
      `[AGENT] Updated message history. Total messages: ${this.messages.length}`,
    );

    if (this.newMessagesBuffer.length > 1) {
      logger.info(
        "[AGENT] ↩️  Aborting reply: context changed in more than 1 message",
      );
      logger.debug(
      ...
      if (sendIntent.isError) {
        logger.error(
          "[AGENT] Failed to send message:",
          sendIntent.error.message,
        );
      } else {
    ...
    otherMessages = this.newMessagesBuffer.filter(
      (msg) => msg.authorId !== this.agentId,
    );

    if (
      Math.random() <= personality.idle_probability ||
      otherMessages.length === 0
    ) {
      logger.debug(`[AGENT] Going idle for ${IDLE_TIMEOUT_MS}ms`);
      this.timeout = setTimeout(
        this._processMessageBatch.bind(this),
        IDLE_TIMEOUT_MS,
      );
  ...
  private async _printProfile() {
    ...
    logger.info(
      `[AGENT] 👤 #${this.agentId} - ${agentName} — impact ${profile.data.impact}`,
    );
  ...
  private async _attachRoomListener() {
    ...
    this.verseClient.subscribeNotification(
